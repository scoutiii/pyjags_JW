...
def convert_pyjags_sample_dictionary_to_arviz_sample_dictionary(
...
def convert_pyjags_samples_dict_to_arviz_inference_data(
    ...
    return az.convert_to_inference_data(
                convert_pyjags_sample_dictionary_to_arviz_sample_dictionary(
                    samples))


def extract_samples_dictionary_from_arviz_inference_data(idata) \
        -> tp.Dict[str, np.ndarray]:
    """
...
def convert_arviz_sample_dictionary_to_pyjags_sample_dictionary(
        samples: tp.Dict[str, np.ndarray]) \
        -> tp.Dict[str, np.ndarray]:
    """
...
def from_pyjags(posterior: tp.Dict[str, np.ndarray]) -> az.InferenceData:
    return convert_pyjags_samples_dict_to_arviz_inference_data(posterior)
